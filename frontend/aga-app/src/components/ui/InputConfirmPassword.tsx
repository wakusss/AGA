interface InputConfirmPasswordProps {
  className?: string;
  confirmPassword: string;
  setConfirmPassword: (value: string) => void;
  passwordValue: string;
}

export default function InputConfirmPassword(props: InputConfirmPasswordProps) {
  const isMismatch =
    props.confirmPassword !== "" &&
    props.passwordValue !== "" &&
    props.confirmPassword !== props.passwordValue;

  return (
    <>
      <div className="grid grid-cols-1">
        <label
          htmlFor="confirmPassword"
          className="text-[var(--color-text-primary-light-mode)] text-xl tracking-wide font-medium ml-2"
        >
          Confirm Password:
        </label>
        <div className="input-wrapper">
          <input
            type="password"
            value={props.confirmPassword}
            onChange={(e) => props.setConfirmPassword(e.target.value)}
            placeholder="Confirm Password"
            className={`md:w-60 rounded-lg m-2 p-3 placeholder:italic text-base tracking-wide text-[var(--color-text-primary-light-mode)] placeholder:text-[var(--color-primary-light-mode)]/35 border-[1.5px] border-[var(--color-primary-light-mode)] focus:outline-none active:brightness-90 hover:brightness-130 hover:saturate-125 transition-all duration-150 shadow-lg ${props.className || ""} ${isMismatch ? "invalid" : ""}`}
            autoComplete="new-password"
            required
          />
        </div>
      </div>
    </>
  );
}
